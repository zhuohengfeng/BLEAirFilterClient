...
public class Utils {
    ...
    public static byte[] stringToHexBytes(String inputStr) {
    ...
    public static String macToString(String mac){
    	Constant.Log("macToString  input="+mac);
    	String masStr = null;
		char[] macArrs= mac.toCharArray();
		// Constant.Log("macToString  macArrs="+macArrs.length);
		char[] macArrsResult = new char[12];
		int i = 0;
		for(char c : macArrs){
			if(c != ':'){
				macArrsResult[i] = c;
		...
		masStr = String.valueOf(macArrsResult);
		Constant.Log("macToString  ouput="+masStr);
		return masStr;
    }
